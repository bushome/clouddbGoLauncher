...
|  $$$$$$ | $$  | $$| $$      | $$|_  $$_/   /$$__  $$      | $$$$$    \  $$$$/ | $$$$$   
 ...
 /$$  \ $$| $$/$$ $$| $$      | $$  | $$ /$$| $$_____/      | $$       /$$/\  $$| $$      
|  $$$$$$/|  $$$$$$/| $$$$$$$$| $$  |  $$$$/|  $$$$$$$      | $$$$$$$$| $$  \ $$| $$$$$$$$
 \______/  \____ $$$|________/|__/   \___/   \_______/      |________/|__/  |__/|________/
                \__/                                                                      
```
You will first need to download the staging dependency repos at https://github.com/bushome/clouddbGO and  https://github.com/bushome/ark-cloud-storage-no-overflow

# clouddbGo Launcher — Build Instructions

This is the Go source for the solo-player/SQLite portable launcher
Two other trees are prerequisites:

- `C:\ARK CLOUD DEDICATED STORAGE API - No OverFlow\clouddbGo\` — build this first if you haven't already
  (`npm install`, `npm run prisma:generate`, `npm run build`).
- `C:\ARK CLOUD DEDICATED STORAGE API - No OverFlow\ark-cloud-storage-no-overflow\` — the dev-tree repo, needed only
...
`C:\ARK CLOUD DEDICATED STORAGE API - No OverFlow\clouddbGoLauncher\`, so it doesn't get tangled in `clouddbGo`'s own
...
```powershell
cd "C:\ARK CLOUD DEDICATED STORAGE API - No OverFlow\clouddbGoLauncher"
.\sync-payload.cmd
```

This copies `dist\` and `generated\sqlite-client\` from `clouddbGo`,
`watchdog\` from the **dev-tree repo root**
(`C:\ARK CLOUD DEDICATED STORAGE API - No OverFlow\ark-cloud-storage-no-overflow\watchdog` — not `clouddbGo`, since
...
`npm ci --omit=dev` inside `C:\ARK CLOUD DEDICATED STORAGE API - No OverFlow\clouddbGo` itself — only against a copy
...
re-extract when that string changes.
...
- **No graceful shutdown handling.** Closing the console window will hit the
  same "not equivalent to a clean stop".
...
- **SQLite Resilience (WAL mode, `PRAGMA quick_check`, backup/restore) lives
  in the NestJS app itself** ("keeps the stub's job
